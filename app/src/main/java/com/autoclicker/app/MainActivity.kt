...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateStatus() {
        tvStatus.text = when {
            !isAccessibilityEnabled() -> "الحالة: فعّل إمكانية الوصول ⚠️"
    ...
    private fun isAccessibilityEnabled(): Boolean {
        ...
        val enabled = try { Settings.Secure.getInt(contentResolver, Settings.Secure.ACCESSIBILITY_ENABLED) } catch (e: Exception) { 0 }
        if (enabled == 1) {
